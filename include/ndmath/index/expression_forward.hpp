...
auto make_subindex(index_base<Dims, false, Derived>& i)
noexcept { return subindex<A, B, index_base<Dims, false, Derived>>{i}; }

template <size_t A, size_t B, size_t Dims, class Derived>
CC_ALWAYS_INLINE CC_CONST constexpr
auto make_subindex(const index_base<Dims, false, Derived>& i)
noexcept { return const_subindex<A, B, index_base<Dims, false, Derived>>{i}; }

template <size_t A, size_t B, size_t Dims, class Derived>
CC_ALWAYS_INLINE CC_CONST constexpr
auto make_subindex(const index_base<Dims, true, Derived>& i)
noexcept { return constexpr_subindex<A, B, index_base<Dims, true, Derived>>{i}; }
...
#define nd_define_arithmetic_op(symbol, name)                       \
	...
	auto operator symbol (                                      \
		const index_base<Dims, IsConstexpr1, Derived1>& i1, \
		const index_base<Dims, IsConstexpr2, Derived2>& i2  \
	) noexcept                                                  \
	{                                                           \
		return binary_index_expr<                           \
			name,                                       \
			index_base<Dims, IsConstexpr1, Derived1>,   \
			index_base<Dims, IsConstexpr2, Derived2>    \
		>{i1, i2};                                          \
	}                                                           \
